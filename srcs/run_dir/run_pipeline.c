...
/*   Updated: 2023/10/24 12:38:33 by jaehyji          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	run_pipe(t_tree_node *child, char ***env)
{
	...
	if (child->num_of_child == 1)
	{
		...
		if (c_pro == 0)
		{
			run_command_nonpipe(child->child_list, env);
			exit(g_exit_status);
		}
		else
...
void	run_pipeline(t_tree_node *node, char ***env, t_symbol before)
{
	...
	while (child)
	{
		if (child->symbol == PIPE)
			return (run_pipe(child, env));
		pipe(pipe_fd);
		c_pro = fork();
		if (c_pro == 0)
		{
			pipe_fd_dup(pipe_fd, 1);
			run_command_pipe(child, env);
			exit(g_exit_status);
		}
		else
		{
